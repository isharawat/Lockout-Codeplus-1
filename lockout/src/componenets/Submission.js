...
const Submission =  () => {
  ...
  const submission = useFetch("https://codeforces.com/api/user.status?handle=" + user.codeForces + "&from=1&count=5")
...
points=0;
    ...
    try {
      ...
      res = await Axios.post("http://localhost:3001/lockoutbot/verifycontest", request, { headers })
    ...
    try {
      if (allUser != [] && points != 0) {
        ...
        const update = await Axios.patch(`http://localhost:3001/lockoutbot/update-user/${res.data.data.code[0]._id}`, value, { headers })
        // console.log(update);
        points = 0;
      }
    }
  ...
  try {
   ...
   const reas = await Axios.post("http://localhost:3001/lockoutbot/verifycontest", request, { headers })
  ...
  return (
    <>
      ...
      final.map((obj)=>(
        <Leaderboard key={obj._id} obj={obj}/>
      )) }
           </tbody>
      </table>
    </>
